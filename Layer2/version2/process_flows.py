...
for flow_id, data in flow_iat.items():
    ...
    for idx, width in zip(local_maxima_indices[0], peak_widths):
        plt.axvline(x=x_values[idx], color='green', linestyle='--', label="Peak Width" if idx == local_maxima_indices[0][0] else None)
        plt.axvline(x=x_values[idx] - width/2, color='green', linestyle=':')
        plt.axvline(x=x_values[idx] + width/2, color='green', linestyle=':')

    # Plot the histogram of the original data
    plt.hist(iat_values, bins=unique_iat, density=True, alpha=0.5, label="Data Histogram")
    ...
    plt.text(0.5, 0.8, "Average Width:"+ str(average_width), transform=plt.gca().transAxes)
